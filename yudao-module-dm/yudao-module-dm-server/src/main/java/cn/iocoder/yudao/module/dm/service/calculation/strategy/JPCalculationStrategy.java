...
@Slf4j
@Component
public class JPCalculationStrategy extends AbstractCountryCalculationStrategy {

    private static final String COUNTRY_CODE = "JP";
    private static final String COUNTRY_NAME = "日本";
    ...
    protected void calculateTaxCosts(ProfitCalculationSaveReqVO request,
        ...
        // 2. 消费税计算（日本消费税率10%）
        calculateConsumptionTax(request, template, result);
    ...
    /**
     * 计算消费税
     * 日本消费税计算公式：售价-售价/(1+消费税率)
     */
    private void calculateConsumptionTax(ProfitCalculationSaveReqVO request,
                                          ProfitCalculationTemplateDO template,
                                          ProfitCalculationResultDTO result) {
        // 获取消费税率
        BigDecimal vatRate = request.getVatRate() != null ? request.getVatRate() : BigDecimal.valueOf(10); // 日本默认10%
        ...
        // 日本消费税计算：售价-售价/(1+消费税率)
        BigDecimal taxRateDecimal = vatRate.divide(BigDecimal.valueOf(100), SCALE_RATE, DEFAULT_ROUNDING);
        BigDecimal divisor = BigDecimal.ONE.add(taxRateDecimal);
        
        BigDecimal consumptionTaxCost = result.getPrice()
                .subtract(result.getPrice().divide(divisor, SCALE_MONEY, DEFAULT_ROUNDING))
                .setScale(SCALE_MONEY, DEFAULT_ROUNDING);
        
        result.setVatCost(consumptionTaxCost);
    }
    ...
    protected void calculateOtherCosts(ProfitCalculationSaveReqVO request,
        ...
        // 日本站点退换货费用计算不包含数字服务费（因为没有数字服务费）
        calculateReturnCostWithoutDigitalService(request, template, result);
    }

    /**
     * 计算不包含数字服务费的退换货费用
     * 退换货费用 = (采购成本+国内运费+货代费+关税+消费税+海运费+配送费+仓储+亚马逊抽成*20%)*退货率
     */
    ...
    public String getCalculationDescription() {
        return String.format(
            ...
            "\n" +
            "3. 利润计算：\n" +
            "   - 利润 = 售价 - 采购成本 - 国内运费 - 货代费 - 关税 - 消费税 - 海运费 - 配送费 - 仓储费 - 亚马逊抽成 - 广告 - 退换货 + 退税\n" +
            "   - 利润率 = 利润 ÷ 售价\n" +
            "   - 投资回报率 = 利润 ÷ (采购成本+国内运费+货代费+关税+消费税+海运费)\n" +
            "\n" +
            "注意：\n" +
            "   - 日本站点无数字服务费\n" +
            "   - 海运费计算公式待定制化\n" +
